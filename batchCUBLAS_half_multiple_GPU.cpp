...
#include "omp.h"
#include <sstream>
const char *sSDKname = "QCT_CUBLAS_BENCHMARK";
using namespace std;
//============================================================================================
...
//============================================================================================
#define CUBLAS_HGEMM_MAX_ULP_ERR    (.1)
#define CUBLAS_HGEMM_MAX_RELATIVE_ERR    (6.e-3)
...
static inline cublasStatus_t cublasXgemm(cublasHandle_t handle,
                                         ...
                                         __half *alpha, const __half *A, int lda,
                                         __half *B, int ldb, __half *beta,
                                         __half *C, int ldc)
{
...
int test_gemm_loop(struct gemmOpts &opts, float err, double max_relative_error, cublasHandle_t handle)
{
    ...
    printf("There are %d GPUs on runing\n",num_gpus);
	

#pragma omp parallel num_threads(num_gpus)
{   
    ostringstream GPULOG;
	...
	*/
	...
	if(errors)
	GPULOG<< "@@@@ "<<opts.elem_type<<"gemm test "<<"FAIL\n";
	else
	GPULOG<< "@@@@ "<<opts.elem_type<<"gemm test "<<"OK\n";
 //////////////HGEMM with SGEMM ///////////////////////
 if ( strcmp ( opts.elem_type ,"s" )==0)
 {	    
		...
		__half *dH_A, *dH_B, *dH_C;
		cudaMallocManaged(&dH_A, opts.N * matrixSizeA * sizeof(__half));
		cudaMallocManaged(&dH_B, opts.N * matrixSizeB * sizeof(__half));
		cudaMallocManaged(&dH_C, opts.N * matrixSizeC * sizeof(__half));
...
while (TESTGEN(gemm)(&opts, matrixM, matrixN, matrixK, numTests, &params) == 0)
    {
        ...
        for (int i = 0; i < opts.N ; i++)
        {
            for( int j = 0; j < matrixSizeA ; j++)
				dH_A[j] = approx_float_to_half(A[j]);
			for( int j = 0; j < matrixSizeB ; j++)
				dH_B[j] = approx_float_to_half(B[j]);
			for( int j = 0; j < matrixSizeA ; j++)
        ...
        start = second();
            ...
            for (int i = 0; i < opts.N ; i++)
            {
				...
				cublasSetStream(handle, streamArray[i]);
				status1 = cublasHgemm(handle, params.transa, params.transb, params.m, params.n,
                          params.k, alpha , dH_A+(i*opts.N), rowsA,
                          dH_B+(i*opts.N), rowsB, beta, dH_C+(i*opts.N), rowsC);
